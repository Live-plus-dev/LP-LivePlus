...
import { getPatientDetailsModel } from '@/models/PatientDetails'
...
export async function connectDB(tenantPath) {
  try {
    ...
    getPatientDetailsModel(tenantConnection);
    getSubscriptionModel(tenantConnection);
...
export function getTenantModel(tenantConnection, modelName) {
  switch (modelName) {
    ...
    case 'Subscription':
      return getSubscriptionModel(tenantConnection);
    default:
